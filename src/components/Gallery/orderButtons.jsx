...
import { moveImageUp, moveImageDown } from "@/app/actions/GalleryData";
import { useRouter } from "next/navigation";

export function ArrowButtons({ imageName, club, isFirst, isLast }) {
  const router = useRouter();

  const handleMoveUp = async () => {
    await moveImageUp(imageName, club);
    router.refresh(); // Force refresh immediately
  };

  const handleMoveDown = async () => {
    await moveImageDown(imageName, club);
    router.refresh(); // Force refresh immediately
  };
